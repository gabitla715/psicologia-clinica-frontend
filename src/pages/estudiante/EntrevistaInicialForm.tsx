...
// Campos opcionales agrupados por sección, usados para el panel de resumen
// en vivo (conteo de avance). Los campos obligatorios (motivoConsultaDetallado,
// riesgoDetectado, fechaEntrevista) se muestran aparte, no en estos contadores.
const SECCIONES_RESUMEN: { etiqueta: string; campos: (keyof Formulario)[] }[] = [
  {
    etiqueta: 'Datos personales',
    campos: ['fechaNacimiento', 'lugarNacimiento', 'estadoCivil', 'ocupacion', 'nivelInstruccion'],
  },
  {
    etiqueta: 'Motivo de consulta',
    campos: ['inicioproblema', 'factoresPrecipitantes', 'intentosSolucion'],
  },
  {
    etiqueta: 'Historia',
    campos: ['historiaPersonal', 'historiaFamiliar', 'relacionesInterpersonales'],
  },
  {
    etiqueta: 'Examen del estado mental',
    campos: ['aparienciaGeneral', 'estadoAfectivo', 'pensamiento', 'percepcion', 'memoriaAtencion', 'juicioCritico'],
  },
  {
    etiqueta: 'Evaluación de riesgo',
    campos: ['observacionesRiesgo'],
  },
  {
    etiqueta: 'Diagnóstico y plan',
    campos: ['impresionDiagnostica', 'recomendaciones'],
  },
];
...
export function EntrevistaInicialForm() {
  ...
  const resumenSecciones = useMemo(
    () =>
      SECCIONES_RESUMEN.map((s) => ({
        etiqueta: s.etiqueta,
        llenos: s.campos.filter((c) => String(form[c] ?? '').trim().length > 0).length,
        total: s.campos.length,
      })),
    [form],
  );
  const totalOpcionales = resumenSecciones.reduce((acc, s) => acc + s.total, 0);
  const totalOpcionalesLlenos = resumenSecciones.reduce((acc, s) => acc + s.llenos, 0);
  const porcentajeAvance =
    totalOpcionales > 0 ? Math.round((totalOpcionalesLlenos / totalOpcionales) * 100) : 0;
  const motivoCompleto = form.motivoConsultaDetallado.trim().length > 0;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-4 grid gap-6 print:mt-0 print:block lg:grid-cols-[320px_1fr] lg:items-start">
        <div className="space-y-4 print:hidden lg:sticky lg:top-6">
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between text-xs text-slate-500">
                ...
                <span className="font-medium text-slate-700">
                  {totalOpcionalesLlenos}/{totalOpcionales} campos
                </span>
              </div>
            ...
            <div className={`mt-4 rounded-lg px-3 py-2.5 ring-1 ${COLOR_NIVEL_RIESGO[form.riesgoDetectado]}`}>
              <p className="text-[10px] font-semibold uppercase tracking-wide opacity-70">
                Nivel de riesgo seleccionado
              </p>
              <p className="mt-0.5 text-sm font-bold">{NOMBRE_NIVEL_RIESGO[form.riesgoDetectado]}</p>
            </div>

            <ul className="mt-4 space-y-2 border-t border-slate-100 pt-3">
              {resumenSecciones.map((s) => (
                <li key={s.etiqueta} className="flex items-center justify-between text-xs">
            ...
            <div className="mt-4 flex items-center gap-2 border-t border-slate-100 pt-3 text-xs">
              ...
              <span className={motivoCompleto ? 'text-slate-600' : 'font-medium text-amber-700'}>
                Motivo de consulta {motivoCompleto ? 'completado' : 'obligatorio pendiente'}
              </span>
            </div>
          </div>
